...
(async()=>{try{
 ...
 await stop();await start('pending@example.test');assert.equal((await api('/admin/data/registrations',{cookie:adminCookie})).status,401);const unverified=await login('pending@example.test');assert.equal(unverified.status,403);assert.match((await unverified.json()).message,/email has not been verified/);ok('Configured but unverified email cannot administer');
