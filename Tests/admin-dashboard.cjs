...
accounts.push({id:'legacy-owner',displayName:'Original archive',email:'archive@botgc.test',normalizedEmail:'ARCHIVE@BOTGC.TEST',passwordHash:hash(),emailVerifiedAt:null,clubId:'legacy',role:'owner',hasUnlimitedTrophyCredits:true,createdAt:now,securityVersion:1});
...
(async()=>{try{
 ...
 const deniedOwner = await login('customer@example.test');assert.equal(deniedOwner.status,403);assert.match((await deniedOwner.json()).message,/not the configured site owner/);
 assert.equal((await login('pending@example.test')).status,403);
 ...
 const list=await api('/admin/data/registrations',{cookie:adminCookie});const text=await list.text();assert.equal(JSON.parse(text).length,4);assert(!/passwordHash|securityVersion|token|storedName/i.test(text));
 ...
 await p.goto(base+'/admin');await p.waitForLoadState('networkidle');assert.equal(await p.locator('#message').innerText(),'');await p.locator('input[name=email]').fill('owner@example.test');await p.locator('input[name=password]').fill(password);await p.locator('#login button').click();await p.locator('#dashboard').waitFor({state:'visible'});
 assert.equal(await p.locator('#registrations tr').count(),4);assert.equal(await p.locator('#registrations script').count(),0);
 await p.locator('#registrations tr').filter({hasText:'Club customer'}).getByRole('button',{name:'View uploads'}).click();await p.locator('#gallery').waitFor({state:'visible'});assert.equal(await p.locator('.photos img').count(),2);await p.waitForFunction(()=>[...document.querySelectorAll('.photos img')].every(i=>i.complete&&i.naturalWidth>0));
 ...
 await stop();await start('pending@example.test');assert.equal((await api('/admin/data/registrations',{cookie:adminCookie})).status,401);const unverified=await login('pending@example.test');assert.equal(unverified.status,200);assert.equal((await api('/admin/data/registrations',{cookie:cookie(unverified)})).status,200);assert.equal((await login('pending@example.test',{data:{email:'pending@example.test',password:'wrong'}})).status,401);const stored=JSON.parse(fs.readFileSync(path.join(root,'identity.json'),'utf8'));assert(!stored.accounts.find(a=>a.id==='pending').emailVerifiedAt);ok('Explicitly configured unverified account can administer with correct password without changing verification status');
 await stop();await start('');const disabled=await login();assert.equal(disabled.status,503);assert.match((await disabled.json()).message,/has not been configured/);assert.equal((await api('/admin/data/registrations',{cookie:adminCookie})).status,401);ok('Missing configuration disables access');
 await stop();await start('archive@botgc.test');const original=await login('archive@botgc.test');assert.equal(original.status,200);assert.equal((await api('/admin/data/registrations',{cookie:cookie(original)})).status,200);assert.equal((await login('pending@example.test')).status,403);ok('Configured original archive account signs in with its existing password');
