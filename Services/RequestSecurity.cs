...
public static class RequestSecurity
{
    public static bool IsSameOriginMutation(HttpRequest request, IConfiguration configuration)
    {
        ...
        // Blog receivers authenticate their own bearer tokens; AutoSEO also supports a raw-body signature.
        if (request.Path == "/api/billing/webhook" || request.Path == BlogEndpoints.WebhookPath || request.Path == BlogPublishingEndpoints.WebhookPath) return true;
