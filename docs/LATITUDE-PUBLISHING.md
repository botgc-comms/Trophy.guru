...
4. It publishes text-only articles at `/blog/{slug}` using Trophy Guru's layout, metadata, canonical URLs, related links and structured data. No provider images are fetched. The normal blog index and sitemap include the post automatically; the existing IndexNow monitor handles public discovery notifications where configured.
...
The documented API has no revision timestamp. Each source ID is imported once, including across app restarts. Provider edits do not overwrite later website/editorial edits. A distinct provider ID cannot replace an existing slug. Deleting/revoking an article in Latitude does not remove the public website article. Landing-page creation is not supported by this importer; it publishes blog posts only.

## Status

The public `/health` response includes `latitude.configured`, `enabled`, `state`, `lastCheckedAt`, and the most recent completed batch's imported/skipped counts. It never includes keys, provider bodies, article identifiers or account details.
