...
Original owner email, password hash, account ID, club ID and unlimited allowance remain supported. The trusted original owner is exempt from the new email-verification gate, including its `.test` address. Opening the original archive must never reassign a different account. Keep the same data-protection application name and key-ring so existing compatible sessions survive.
...
The public endpoint stays public inside a protected CMS page. For members-only Intelligent Golf access, first obtain a supported OIDC/SAML flow or server-issued signed member assertion, or use an explicitly approved protected-content synchronisation arrangement. Reading `window.userID`, page text or member names in browser JavaScript is not proof of authentication. Do not embed a signing secret or privileged API key in a page/CDN file. The supplied BOTGC page already uses centrally hosted scripts and a member ID for personalisation. Security correction: the inspected `BOTGC.API/Program.cs` excludes `/api/cms/pages` from API-key middleware, leaving the HTML/Markdown publishing actions without independent authentication. The existing browser trophy APIs also use a shared website key without verified member identity or administrator checks on some mutations. Both are release blockers for reusing that service; deployed exposure has not been tested. See [security audit](SECURITY-AUDIT-2026-09-06.md). The proposed members-only pilot instead places an approved snapshot inside a restricted CMS page, including protected artwork, and must verify logged-out access is denied. Snapshot withdrawal must update the installed CMS copy too. See [Intelligent Golf integration design](INTELLIGENT-GOLF-INTEGRATION.md) for the reference audit and remaining implementation work.
