...
6 September 2026. The current local implementation passes 54 automated regression cases and the isolated browser smoke checks; this is not a record of a production deployment or real payment. Keep billing disabled until the external release gates below have evidence.
...
| Optional integration subscription | Leave `IG_INTEGRATION_AVAILABLE=false` and `STRIPE_IG_PRICE_ID` blank until the contracted integration is deliverable. Recurring checkout/portal code alone is not an Intelligent Golf login adapter. |
